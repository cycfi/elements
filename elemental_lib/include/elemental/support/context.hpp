/*=============================================================================
   Copyright (c) 2016-2019 Joel de Guzman

   Distributed under the MIT License [ https://opensource.org/licenses/MIT ]
=============================================================================*/
#if !defined(CYCFI_ELEMENTAL_GUI_LIB_CONTEXT_APRIL_17_2016)
#define CYCFI_ELEMENTAL_GUI_LIB_CONTEXT_APRIL_17_2016

#include <elemental/support/point.hpp>
#include <elemental/support/rect.hpp>
#include <cairo.h>

namespace cycfi { namespace elemental
{
	////////////////////////////////////////////////////////////////////////////////////////////////
	// Contexts
	////////////////////////////////////////////////////////////////////////////////////////////////
   class view;
   class element;
   class canvas;

   struct basic_context
   {
      basic_context(elemental::view& view_, elemental::canvas& canvas_)
       : view(view_)
       , canvas(canvas_)
      {}

      basic_context(basic_context const&) = default;
      basic_context& operator=(basic_context const&) = default;

      elemental::view&        view;
      elemental::canvas&      canvas;
   };

	////////////////////////////////////////////////////////////////////////////////////////////////
   struct context : basic_context
   {
      context(context const& rhs, elemental::rect bounds_)
       : basic_context(rhs.view, rhs.canvas), element(rhs.element)
       , parent(rhs.parent), bounds(bounds_)
      {}

      context(context const& parent_, element* element_, elemental::rect bounds_)
       : basic_context(parent_.view, parent_.canvas), element(element_)
       , parent(&parent_), bounds(bounds_)
      {}

      context(class view& view_, class canvas& canvas_, element* element_, elemental::rect bounds_)
       : basic_context(view_, canvas_), element(element_)
       , parent(0), bounds(bounds_)
      {}

      context(context const&) = default;
      context& operator=(context const&) = default;

      elemental::element*  element;
      context const*    parent;
      elemental::rect      bounds;
   };
}}

#endif
