...
What Elements needs depends on the graphics backend, and the backend
depends on the platform.

On MacOS, Elements draws with *Quartz2D*, which is part of the operating
system. There are no libraries to install and no package manager to set
up: a C{pp} compiler and CMake are enough.

Elsewhere, Elements draws with *Cairo*, and needs it and its companions:
...
. https://harfbuzz.github.io/[harfbuzz]

On Linux, the GTK host additionally needs GTK3.
...
There are none. Elements draws with Quartz2D on MacOS, which comes with
the system, so nothing needs to be installed and pkg-config is not
required.

You do need the command line developer tools, for the compiler and the
system headers. If `git` or `clang` is missing, this installs them:
