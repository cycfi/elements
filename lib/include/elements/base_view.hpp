...
namespace cycfi { namespace elements
{
   ...
   public:
      ...
      virtual void      draw(cairo_t* ctx, rect area) { ignore(ctx, area); };
      virtual void      click(mouse_button btn) { ignore(btn); }
      virtual void      drag(mouse_button btn) { ignore(btn); }
      virtual void      cursor(point p, cursor_tracking status) { ignore(p, status); }
      virtual void      scroll(point dir, point p) { ignore(dir, p); }
      virtual void      key(key_info const& k) { ignore(k); }
      virtual void      text(text_info const& info) { ignore(info); }
